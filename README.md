...
- **Viewport-filling layout**: Dashboard root uses `min-h-[calc(100dvh-15rem)]` (accounts for sticky header + dock) with a unified `flex-1` CSS grid (`lg:grid-rows-[1fr_1fr]`) that distributes Campaigns, Checklist, and Team Status equally across the viewport. All three cards are flex columns with fixed headers and `overflow-y-auto` scrollable content areas (thin scrollbar), so they fill the screen down to the dock without compressing or overflowing
- **Campaigns + Checklist**: campaigns card on the left (lg:col-span-5, row 1), checklist on the right (lg:col-span-7, row 1). Both have inner vertical scroll for any number of items. Checklist uses card-background rows (matching campaigns style) with priority icon, priority badge, status pill (Pending/In Progress), assigned-to name, created-by name, deadline, and created date. Both sections always show their card shell with skeleton rows while data loads
...
- **Team Status (Live Presence)** (lg:col-span-12, row 2, inner vertical scroll): pulsing green dot header, segmented pill filter (All/Office/Remote/Late/Absent), animated employee cards with gradient avatars, breathing ring animations for live employees, `badge-*` status pills, live/flagged badges. Card layout: **Clock In · Hours · Clock Out** 3-column row (work start to last session end), **Arrived · Office · Left** 3-column row (first office entry to last office exit, shown only when office time > 0), **Activity Strip** with segmented time bar (green=office, blue=remote, purple=break proportions within shift, shift % label that turns green at 100%), compact detail chips showing session count with repeat icon, remote/break time (shown when > 0), late penalty (conditional), and idle gap time (shown when > 5min, computed from clock-in-to-out span minus working minutes), shift progress bar, location flag detail box when flagged, and **always-visible footer** showing pending tasks, active tasks, and campaign count (shown as "0 pending / 0 active / 0 campaigns" when empty)
- **Stale session detection**: Presence API checks `lastActivity` against a 3-minute threshold. Stale employees show as inactive with "Last seen [time]". Priority chain: stale session `lastActivity` → `DailyAttendance.lastSessionEnd` (tracks end of most recent session regardless of location) → `lastOfficeExit` fallback. This ensures remote workers who left the office at 10:59 but continued remotely until 11:45 show "Last seen 11:45" instead of the stale office exit. Total time for stale sessions correctly includes the unclosed session's elapsed minutes
