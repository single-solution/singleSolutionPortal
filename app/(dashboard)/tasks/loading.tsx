    ...
    <div className="flex flex-col gap-0 animate-reveal">
      <div className="mb-4 flex items-center justify-between gap-3">
        <div>
          ...
          <div className="shimmer mt-1 h-3 w-36 rounded" />
        </div>
        <div className="flex items-center gap-0.5 rounded-lg border p-0.5" style={{ background: "var(--bg)", borderColor: "var(--border-strong)" }}>
          <span className="rounded-md px-2.5 py-1 text-xs font-semibold" style={{ background: "var(--bg-elevated)", color: "var(--fg)" }}>Latest</span>
          <span className="rounded-md px-2.5 py-1 text-xs font-medium" style={{ color: "var(--fg-tertiary)" }}>Deadline</span>
          <span className="rounded-md px-2.5 py-1 text-xs font-medium" style={{ color: "var(--fg-tertiary)" }}>Priority</span>
        </div>
      </div>
      <div className="card-static mb-3 flex items-center gap-3 p-3">
        <input className="input flex-1" placeholder="Search tasks..." disabled readOnly />
        <span className="btn btn-primary shrink-0">Create Task</span>
      </div>
      <div className="mb-3 flex flex-wrap items-center gap-2">
        <div className="flex flex-wrap items-center gap-0.5 rounded-lg border p-0.5" style={{ background: "var(--bg)", borderColor: "var(--border-strong)" }}>
          {["All", "Low", "Medium", "High", "Urgent"].map((label) => (
