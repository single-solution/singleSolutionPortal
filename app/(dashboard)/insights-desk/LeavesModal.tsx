      ...
      <AnimatePresence>
        {open && (
          ...
          >
            ...
            <motion.div
              className={`relative mx-2 sm:mx-6 flex flex-col rounded-xl border shadow-xl overflow-hidden ${showSidebar ? "w-full max-w-6xl h-[calc(100dvh-1rem)] sm:h-[min(80vh,900px)]" : "w-full max-w-3xl h-[calc(100dvh-1rem)] sm:h-[min(80vh,900px)]"}`}
            ...
            >
              ...
              <div className="shrink-0 flex items-center justify-between px-3 py-2 border-b" style={{ borderColor: "var(--border)" }}>
                ...
                <div className="flex items-center gap-2">
                  <div className="flex items-center rounded-lg border" style={{ borderColor: "var(--border)" }}>
                    <button type="button" onClick={() => setSelYear(y => y - 1)} className="px-2.5 py-1 text-[12px] font-bold" style={{ color: "var(--fg-secondary)" }}>‹</button>
                    <span className="px-2 text-[12px] font-bold tabular-nums" style={{ color: "var(--fg)" }}>{selYear}</span>
                    <button type="button" onClick={() => setSelYear(y => y + 1)} disabled={selYear >= new Date().getFullYear()} className="px-2.5 py-1 text-[12px] font-bold disabled:opacity-30" style={{ color: "var(--fg-secondary)" }}>›</button>
                  </div>
                  <motion.button
                    type="button"
              ...
              <div className="flex flex-1 overflow-hidden">
                ...
                {showSidebar && (
                  <div
                    className="hidden sm:flex flex-col border-r w-[260px] shrink-0"
                  ...
                  >
                    ...
                    <div className="flex-1 overflow-y-auto py-1.5">
                      ...
                      {!sidebarSearch && (
                        ...
                        >
                          <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-[12px] font-bold" style={{ background: "color-mix(in srgb, var(--primary) 15%, transparent)", color: "var(--primary)" }}>
                            {isSuperAdmin ? "All" : "You"}
                          </span>
                          <span className="text-[12px] font-semibold" style={{ color: !userId && !deptFilter ? "var(--primary)" : "var(--fg-secondary)" }}>
                          ...
                          {g.employees.map((emp) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <span className="flex-1 min-w-0 text-[12px] font-medium truncate" style={{ color: isSel ? "var(--primary)" : "var(--fg)" }}>
                                  {nameOf(emp)}
                                </span>
                      ...
                      {filteredEmployees.length === 0 && sidebarSearch && (
                        <p className="px-3 py-4 text-center text-[12px]" style={{ color: "var(--fg-tertiary)" }}>No matches</p>
                      )}
                      </>}
                    </div>

                    <div className="border-t px-3 py-2" style={{ borderColor: "var(--border)" }}>
                      <p className="text-[12px] font-medium" style={{ color: "var(--fg-tertiary)" }}>
                  ...
                  {allMode ? (
                    <div className="space-y-4">
                      ...
                      <div className="flex items-center gap-3 rounded-xl p-3" style={{ background: "var(--bg-grouped)" }}>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[12px]" style={{ color: "var(--fg-tertiary)" }}>Organization-wide leave overview · {selYear}</p>
                        </div>
                      </div>
                      {/* All-employees tabs */}
                      <div className="flex gap-1 rounded-lg border p-0.5" style={{ borderColor: "var(--border)" }}>
                        {(["overview", "employees"] as const).map((t) => (
                          <button key={t} type="button" onClick={() => setLeaveTab(t)}
                            className={`flex-1 rounded-lg px-3 py-1.5 text-[12px] font-semibold transition-all ${leaveTab === t ? "bg-[var(--primary)] text-white shadow-sm" : "text-[var(--fg-secondary)]"}`}
                          >
                        ...
                        {leaveTab === "overview" && (
                          <motion.div key="all-overview" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }} transition={{ duration: 0.15 }} className="space-y-4">
                            {leavesLoading ? (
                              ...
                              <p className="py-8 text-center text-[12px]" style={{ color: "var(--fg-tertiary)" }}>No leave records for {selYear}.</p>
                            ) : (
                              <>
                                  ...
                                  <div className="rounded-xl border p-3" style={{ borderColor: "var(--border)" }}>
                                    ...
                                    <div className="flex flex-wrap gap-1.5">
                                      {leaveSummary.typeDayBreakdown.map((t) => (
                                        <span key={t.label} className="rounded-full border px-2.5 py-0.5 text-[12px] font-semibold tabular-nums" style={{ borderColor: "var(--border)", color: "var(--fg-secondary)" }}>
                                          {t.label}: {t.days}d
                                        </span>
                                      ))}
                                    </div>
                                ...
                                {leaves.length > 0 && (
                                  <div className="mt-2 border-t pt-3" style={{ borderColor: "var(--border)" }}>
                                    <p className="text-[12px] font-semibold uppercase tracking-wider mb-2" style={{ color: "var(--fg-tertiary)" }}>Leave History · {selYear}</p>
                                          ...
                                          <div key={l._id} className="flex items-center justify-between gap-2 rounded-lg px-2.5 py-1.5" style={{ background: "var(--bg-grouped)" }}>
                                            <div className="min-w-0 flex-1">
                                              <div className="flex items-center gap-1.5">
                                                ...
                                                <span className="truncate text-[12px] font-semibold" style={{ color: "var(--fg)" }}>{empName || "—"}</span>
                                                <span className="shrink-0 text-[12px] tabular-nums" style={{ color: "var(--fg-tertiary)" }}>{l.days}d</span>
                                              </div>
                                      ...
                                      {allEmployeeSummary.rows.map((r, i) => (
                                        <tr key={i} className="border-t" style={{ borderColor: "var(--border)" }}>
                                          <td className="py-1.5">
                                            ...
                                            {r.dept && <p className="text-[12px]" style={{ color: "var(--fg-tertiary)" }}>{r.dept}</p>}
                                          </td>
                      ...
                      return (
                        <div className="space-y-4">
                          <div className="flex items-center gap-3 rounded-xl p-3" style={{ background: "var(--bg-grouped)" }}>
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="text-[12px] font-bold" style={{ color: "var(--fg)" }}>{deptName}</p>
                              <p className="text-[12px]" style={{ color: "var(--fg-tertiary)" }}>{deptEmpIds.size} employee{deptEmpIds.size !== 1 ? "s" : ""} · {selYear}</p>
                            </div>
                          </div>
                          {deptLeaves.length === 0 ? (
                            <p className="py-8 text-center text-[12px]" style={{ color: "var(--fg-tertiary)" }}>No leave records for this department in {selYear}.</p>
                          ) : (
                            <>
                    ...
                    <>
                      ...
                      {userId && selectedEmployee && (
                        <div className="flex items-center gap-3 rounded-xl p-3" style={{ background: "var(--bg-grouped)" }}>
                          <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-[12px] font-bold text-white" style={{ background: avatarColor(selectedEmployee._id) }}>
                            {initials(selectedEmployee)}
                          </span>
                          <div className="flex-1 min-w-0">
                            <p className="text-[12px] font-bold truncate" style={{ color: "var(--fg)" }}>{nameOf(selectedEmployee)}</p>
                            {selectedEmployee.department && (
                              <p className="text-[12px]" style={{ color: "var(--fg-tertiary)" }}>{selectedEmployee.department.title}</p>
                            )}
                          </div>
                        </div>
                      )}
                      ...
                      {balLoading ? (
                        ...
                        <div className="rounded-xl p-4 space-y-3" style={{ background: "var(--bg-grouped)" }}>
                          <div className="flex justify-between text-[12px] font-semibold" style={{ color: "var(--fg-tertiary)" }}>
                          ...
                          <div className="grid grid-cols-3 gap-2">
                            <div className="rounded-lg p-2.5 text-center" style={{ background: "var(--bg-elevated)" }}>
                              <p className="text-[12px] font-semibold uppercase tracking-wider" style={{ color: "var(--fg-tertiary)" }}>Total days</p>
                              <p className="text-[12px] font-bold" style={{ color: "var(--primary)" }}>{balance.total}</p>
                            </div>
                            <div className="rounded-lg p-2.5 text-center" style={{ background: "var(--bg-elevated)" }}>
                              <p className="text-[12px] font-semibold uppercase tracking-wider" style={{ color: "var(--fg-tertiary)" }}>Days used</p>
                              <p className="text-[12px] font-bold" style={{ color: barColor }}>{balance.used}</p>
                            </div>
                            <div className="rounded-lg p-2.5 text-center" style={{ background: "var(--bg-elevated)" }}>
                              <p className="text-[12px] font-semibold uppercase tracking-wider" style={{ color: "var(--fg-tertiary)" }}>Days left</p>
                              <p className="text-[12px] font-bold" style={{ color: "var(--green)" }}>{balance.remaining}</p>
                            </div>
                          </div>
                          {balanceRunoutDays != null && (
                            <div className="rounded-lg px-2.5 py-2 text-center" style={{ background: "var(--bg-elevated)" }}>
                              <p className="text-[12px] font-semibold uppercase tracking-wider" style={{ color: "var(--fg-tertiary)" }}>Days Until Balance Runs Out</p>
                              <p className="text-[12px] font-bold" style={{ color: "var(--fg-secondary)" }}>~{balanceRunoutDays} days <span className="font-normal" style={{ color: "var(--fg-tertiary)" }}>(est.)</span></p>
                            </div>
                          )}
                        </div>
                      ) : null}
                      ...
                      {balance && !leavesLoading && leaves.length > 0 && (
                        <div className="grid grid-cols-3 gap-2">
                          <div className="rounded-lg p-2 text-center" style={{ background: "var(--bg-grouped)" }}>
                            <p className="text-[12px] font-semibold uppercase tracking-wider" style={{ color: "var(--fg-tertiary)" }}>On Leave Today</p>
                            <p className="text-[12px] font-bold" style={{ color: leavePersonalExtras.onLeaveToday ? "var(--green)" : "var(--fg-tertiary)" }}>
                              {leavePersonalExtras.onLeaveToday ? "Yes" : "No"}
                            </p>
                          </div>
                          <div className="rounded-lg p-2 text-center" style={{ background: "var(--bg-grouped)" }}>
                            <p className="text-[12px] font-semibold uppercase tracking-wider" style={{ color: "var(--fg-tertiary)" }}>Half-Day Leaves</p>
                            <p className="text-[12px] font-bold" style={{ color: "var(--purple)" }}>{leavePersonalExtras.halfDayLeaves}</p>
                          </div>
                          <div className="rounded-lg p-2 text-center" style={{ background: "var(--bg-grouped)" }}>
                            <p className="text-[12px] font-semibold uppercase tracking-wider" style={{ color: "var(--fg-tertiary)" }}>Next Scheduled Leave</p>
                            <p className="text-[12px] font-bold truncate px-0.5" style={{ color: "var(--primary)" }} title={leavePersonalExtras.nextScheduledStart ?? undefined}>
                          ...
                          >
                            <form onSubmit={(e) => void handleSubmit(e)} className="rounded-xl p-4 space-y-3" style={{ background: "var(--bg-grouped)" }}>
                              ...
                              <div>
                                <div className="flex gap-1 rounded-lg border p-0.5" style={{ borderColor: "var(--border)" }}>
                                  <button
                                    ...
                                    className={`flex-1 rounded-lg px-3 py-1.5 text-[12px] font-semibold transition-all ${!isHalfDay ? "bg-[var(--primary)] text-white shadow-sm" : "text-[var(--fg-secondary)]"}`}
                                  >
                                    Full day
                                  </button>
                                  <button
                              ...
                              <div className={multiDay && !isHalfDay ? "grid grid-cols-2 gap-2" : ""}>
                                <label className="flex flex-col gap-1 text-[12px] font-semibold" style={{ color: "var(--fg-tertiary)" }}>
                                  {multiDay && !isHalfDay ? "Start date" : "Date"}
                                  <input type="date" required className="input text-[12px]" value={date} onChange={(e) => setDate(e.target.value)} />
                                </label>
                                {multiDay && !isHalfDay && (
                                  <label className="flex flex-col gap-1 text-[12px] font-semibold" style={{ color: "var(--fg-tertiary)" }}>
                                    End date
                                    <input type="date" required className="input text-[12px]" value={endDate} onChange={(e) => setEndDate(e.target.value)} min={date} />
                                  </label>
                                )}
                              </div>
                              <div className="grid grid-cols-2 gap-2">
                                <label className="flex flex-col gap-1 text-[12px] font-semibold" style={{ color: "var(--fg-tertiary)" }}>
                                  Type
                                  <select className="input text-[12px]" value={leaveType} onChange={(e) => setLeaveType(e.target.value)}>
                                ...
                                <label className="flex flex-col gap-1 text-[12px] font-semibold" style={{ color: "var(--fg-tertiary)" }}>
                                  ...
                                  <input type="text" className="input text-[12px]" value={reason} onChange={(e) => setReason(e.target.value)} placeholder="Personal, health, etc." />
                                </label>
                              </div>
                      ...
                      {/* Leave history */}
                      <div>
                        <p className="text-[12px] font-semibold uppercase tracking-wider mb-2" style={{ color: "var(--fg-tertiary)" }}>
                            ...
                            {leaves.map((l, idx) => {
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <span className="shrink-0 rounded-full px-2 py-0.5 text-[12px] font-semibold capitalize" style={{
