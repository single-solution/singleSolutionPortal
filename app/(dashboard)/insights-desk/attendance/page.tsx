...
export default function AttendancePage() {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {/* Company Holidays */}
      {sessionReady && <HolidaysSection year={year} canManageHolidays={canManageHolidays} />}
    </div>
  );
}
...
function HolidaysSection({ year, canManageHolidays }: { year: number; canManageHolidays: boolean }) {
  ...
  const fetchHolidays = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/payroll/holidays?year=${year}`);
  ...
  async function handleDelete() {
    ...
    try {
      ...
      if (res.ok) { toast.success("Holiday removed"); setDeleteTarget(null); await fetchHolidays(); }
            ...
            <span className="text-sm font-semibold" style={{ color: "var(--fg)" }}>
              ...
              {holidays.length > 0 && (
                <span className="ml-1.5 text-[10px] font-medium" style={{ color: "var(--fg-tertiary)" }}>
                  · {holidays.length} in {year}{upcoming.length > 0 ? ` · ${upcoming.length} upcoming` : ""}
                </span>
              )}
            </span>
          </div>
              ...
              <div className="border-t px-4 py-3 space-y-3" style={{ borderColor: "var(--border)" }}>
                ...
                <AnimatePresence>
                  {showForm && (
                    <motion.div
                      ...
                      className="overflow-hidden"
                    >
                      <div className="rounded-xl p-3 space-y-3" style={{ background: "var(--bg-grouped)" }}>
                ...
                {loading ? (
                  <div className="space-y-2">
                    {[1, 2, 3].map((i) => (
                      <div key={i} className="flex items-center gap-3 rounded-lg p-2" style={{ background: "var(--bg-grouped)" }}>
                        <div className="shimmer h-8 w-8 rounded-lg" />
                        <div className="flex-1 space-y-1"><div className="shimmer h-3 w-32 rounded" /><div className="shimmer h-2.5 w-20 rounded" /></div>
                      </div>
                    ))}
                  </div>
                ) : holidays.length === 0 ? (
                  <p className="text-xs py-2" style={{ color: "var(--fg-tertiary)" }}>No holidays declared for {year}.</p>
                ) : (
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            ...
                            <span className="text-xs font-bold leading-tight">{d.getUTCDate()}</span>
                          </div>
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[10px]" style={{ color: "var(--fg-tertiary)" }}>
                              ...
                              {h.isRecurring && (
                                <span className="ml-1 text-[9px] font-semibold" style={{ color: "#8b5cf6" }}>· Recurring</span>
                              )}
                            </p>
                          </div>
                          {canManageHolidays && (
